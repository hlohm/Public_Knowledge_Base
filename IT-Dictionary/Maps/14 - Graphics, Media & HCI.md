...
- [[Color Space]] — A defined model and range (gamut) for representing colour numerically — sRGB, Display P3, Adobe RGB, CMYK for print.
- [[Font]] — A digital file defining the shapes (glyphs) of characters at a given style and weight.
- [[Lossless Compression]] — Compression that reduces size with no loss of information — the original is reconstructed exactly (ZIP, PNG, FLAC, gzip).
- [[Lossy Compression]] — Compression that permanently discards information judged perceptually unimportant to achieve much smaller files (JPEG, MP3, most video).
- [[Pixel]] — The smallest addressable element of a raster image or display — a single point, usually carrying colour as RGB (and sometimes alpha) values.
- [[Raster]] — Image representation as a grid of [[Pixel]]s — fixed [[Resolution]], scales badly upward, but can depict anything.
- [[Ray Tracing]] — A rendering technique that simulates light by tracing the paths of rays through a scene, producing physically accurate reflections, refractions, and shadows.
